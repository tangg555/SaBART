...
class OneHopGNNBartForCGNoAgg(OneHopGNNBartForCG):
    ...
    def forward(self, one_hop_graph,
        ...
        if encoder_last_hidden_state is None:
            ...
            # --------------
            encoder_last_hidden_state = torch.cat((child_node_knowledge, encoded_outputs[0]), dim=1)  # no post features
...
class OneHopGNNBartNoAgg(MyBart):
    ...
    def _custom_init(self):
        ...
        # model
        self.model = self._load_model(self.hparams.model_name_or_path, OneHopGNNBartForCGNoAgg, self.config)
