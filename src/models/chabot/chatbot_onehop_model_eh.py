...
from src.modules.chatbot.graph_attention import GlobalGraphAttnLayer
...
class OneHopGNNBartForCGEH(OneHopGNNBartForCG):
    def __init__(self, config):
        super().__init__(config)
        self.graph_model_1 = GlobalGraphAttnLayer(config.d_model * 4, config.d_model * 4, config.hidden_size,
                                            self.graph_drop, self.leaky_negative_slope)
        self.cls = nn.Linear(self.config.hidden_size + config.d_model, self.config.hidden_size + config.d_model,  bias=True)
        self.init_weights()
        self.tie_weights()
    ...
    def forward(self, one_hop_graph,
        ...
        if encoder_last_hidden_state is None:
            ...
            cls = self.cls(torch.cat((encoded_outputs[0][:,0], torch.max(child_node_knowledge, dim=1)[0]), dim=1))

            for gindex, g in enumerate(graphs):
                g.ndata['q'] = cls[gindex].expand((g.num_nodes(), self.config.hidden_size + self.config.d_model))

            graph_batch = dgl.batch(graphs)

            graph_batch = self.graph_model_1(graph_batch)
            ...
            encoder_last_hidden_state = torch.cat((one_hop_knowledge_distill, child_node_knowledge, encoded_outputs[0]), dim=1)  # no post features
...
class OneHopGNNBartEH(MyBart):
    ...
    def _custom_init(self):
        ...
        # model
        self.model = self._load_model(self.hparams.model_name_or_path, OneHopGNNBartForCGEH, self.config)
